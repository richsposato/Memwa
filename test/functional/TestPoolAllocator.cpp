...
void AllocatePoolChunks( ut::UnitTest * u, Allocator * allocator, unsigned int chunkCount,
...
void TestPoolAllocator( bool multithreaded, bool showProximityCounts )
{
	...
	std::cout << "Basic Functionality " << threadType << " Pool Allocator Test" << std::endl; 
	ut::UnitTestSet & uts = ut::UnitTestSet::GetIt();
	ut::UnitTest * u = uts.AddUnitTest( "Test Pool Allocator" );
...
//	std::cout << "Allocator Type: " << typeid( *allocator ).name() << std::endl;
...

	{
		...
		AllocatePoolChunks( u, allocator, chunkCount, chunks, allocatorInfo, true );
	...
	{
		ChunkList chunks( chunkCount );
		AllocatePoolChunks( u, allocator, chunkCount, chunks, allocatorInfo, false );
		UNIT_TEST( u, chunks.GetCount() == chunkCount );
		UNIT_TEST( u, chunks.AreUnique() );
...
{
	const char * threadType = ( multithreaded ) ? "Multi-Threaded" : "Single-Threaded";
	std::cout << "Complex Functionality " << threadType << " Pool Allocator Test" << std::endl; 
	ut::UnitTestSet & uts = ut::UnitTestSet::GetIt();
	ut::UnitTest * u = uts.AddUnitTest( "Complex Test Pool Allocator" );
...
{
	std::cout << "Pool Allocator Thread-Safety Functionality Test" << std::endl; 
	ut::UnitTestSet & uts = ut::UnitTestSet::GetIt();
	ut::UnitTest * u = uts.AddUnitTest( "Pool Thread-Safety Test" );
